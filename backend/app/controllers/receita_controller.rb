class ReceitaController < ApplicationController
  ...
  def update
    respond_to do |format|
      if @receitum.update(receitum_params)
        format.html { redirect_to @receitum, notice: "Receitum was successfully updated.", status: :see_other }
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to receita_path, notice: "Receitum was successfully destroyed.", status: :see_other }
  ...
  private
    ...
    def set_receitum
      @receitum = Receitum.find(params.expect(:id))
    end

    # Only allow a list of trusted parameters through.
    def receitum_params
      params.expect(receitum: [ :titulo, :valor, :data, :categoria ])
    end
end
