class GastosController < ApplicationController
  ...
  def update
    respond_to do |format|
      if @gasto.update(gasto_params)
        format.html { redirect_to @gasto, notice: "Gasto was successfully updated.", status: :see_other }
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to gastos_path, notice: "Gasto was successfully destroyed.", status: :see_other }
  ...
  private
    ...
    def set_gasto
      @gasto = Gasto.find(params.expect(:id))
    end

    # Only allow a list of trusted parameters through.
    def gasto_params
      params.expect(gasto: [ :titulo, :valor, :data, :categoria ])
    end
end
